...
fun Fragment.configuration() {
    with(SwiftDownloader) {
        initWithServiceMode(requireActivity().application)
        option.showNotification = true
        val remoteViews = RemoteViews(requireActivity().packageName, R.layout.notify_download)
        val remoteViewsCustom =
            RemoteViews(requireActivity().packageName, R.layout.notify_download_custom)
        notificationSender = object : NotificationSender(requireActivity().application) {
            ...
            override fun buildDownloadProgressNotification(
                ...
                return NotificationCompat.Builder(requireActivity(), DOWNLOAD_CHANNEL_ID)
                ...
                return if(isApkFile(fileName)) {
                    ...
                    if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                        ...
                        val contentUri = FileProvider.getUriForFile(
                            requireActivity(), "${requireActivity().packageName}.fileprovider", file
                        )
                    ...
                    NotificationCompat.Builder(requireActivity(), DOWNLOAD_CHANNEL_ID)
                        .setSmallIcon(R.mipmap.ic_launcher)
                        .setContent(remoteViews)
